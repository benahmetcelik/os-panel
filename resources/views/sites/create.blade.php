...
@section('content')
    <div class="card-area">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('sites.store') }}" class="form">
                @csrf
                @if($errors->any())
                ...
                <div class="grid">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="ssl_status_switch"
                               data-for="ssl_status"
                               value="{{ old('ssl_status',0) }}" {{ old('ssl_status') === '1' ? 'checked' : '' }}>
                        <label class="form-check-label" for="ssl_status_switch"></label>
                        <input type="hidden" name="ssl_status" value="{{ old('ssl_status',0) }}"
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="backup_status_switch"
                               ...
                               value="{{ old('backup_status',0) }}" {{ old('backup_status') === '1' ? 'checked' : '' }}>
                        <label class="form-check-label" for="backup_status_switch"></label>
                        <input type="hidden" name="backup_status" value="{{ old('backup_status',0) }}"
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="enabled_status_switch"
                               ...
                               value="{{ old('enabled_status',0) }}" {{ old('enabled_status') === '1' ? 'checked' : '' }}>
                        <label class="form-check-label" for="enabled_status_switch"></label>
                        <input type="hidden" name="enabled" value="{{ old('enabled_status',0) }}"
